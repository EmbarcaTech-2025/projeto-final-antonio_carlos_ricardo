...
public class IoT_Dummy_SendData {
    static String ip;
    static String porta;
    static String id;
    ...
    static public void getTime(){
        String jsonReq = String.format(Locale.US, "{\"id\":\"%s\",\"token\":\"token_de_teste\"}", id);
        String jsonResp = enviarPost2(ip, porta, jsonReq);
        Instant t_server = null;
        
        if (jsonResp != null) {
            System.out.println("JSON TIMER recebido: " + jsonResp);
            String key = "\"time\":";
            int idx = jsonResp.indexOf(key);
            if (idx >= 0) {
                int start = idx + key.length();
                int end = jsonResp.indexOf('}', start);
                if (end == -1) end = jsonResp.length();    
                String timeStr = jsonResp.substring(start+1, end-1).trim();
                if(timeStr.length() > 0){
                    t_server = Instant.parse(timeStr);
                }
            }
                    
            if(t_server == null) System.out.println("Hora do servidor == NULL");
                            else System.out.println("Hora do servidor: " + t_server.atZone(ZoneId.of("UTC")).format(DateTimeFormatter.ISO_OFFSET_DATE_TIME));
        } else {
            System.out.println("Server Timer não envio time");
        }
        System.out.println();
    }
    
    private static String enviarPost2(String ip, String porta, String json) {
        try {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length < 3) {
            System.out.println("Uso: java IoT_Dummy_SendData <ip> <porta> <id_dispositivo>");
            return;
        }
        ...
        Double  lon        = 123.456789;
        String  group      = "station_group";
        
        try{
            int id_n = Integer.parseInt(id);
            if((id_n >=0) && (id_n <=12)) {
                lat        = (Double)lista_estacoes[id_n][0];
                lon        = (Double)lista_estacoes[id_n][1];
                group      = (String)lista_estacoes[id_n][2];
                deviceName = (String)lista_estacoes[id_n][3];
            }else{
                System.out.println("Número invalido para a estação: " + id_n);
                return;
            }
        } catch (NumberFormatException e){
            System.out.println("Usando: valores default para a estação " + id);
        }
