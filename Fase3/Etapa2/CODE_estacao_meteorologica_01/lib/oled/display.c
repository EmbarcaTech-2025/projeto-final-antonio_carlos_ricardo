...
void    display_scroll_left(bool on, bool update){
    uint16_t point = 0;
    for(int i=0;i<8;i++){
        //memcpy(&display_data[point], &display_data[point+1], 127);
        memmove(&display_data[point], &display_data[point+1], 127);
        display_data[point + 127] = on?0xFF:0x00;
        point += 128;
    }
    if(update) display_update();
}
