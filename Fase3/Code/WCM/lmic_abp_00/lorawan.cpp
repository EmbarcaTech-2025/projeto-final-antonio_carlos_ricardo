...
void         lorawan_send(){
    ...
    switch(lorawan_pars.mode){
        ...
        case LORAWAN_MODE_ABP:
            ...
            devaddr = (uint32_t)lorawan_pars.devaddr[0];
            devaddr = (devaddr << 8) | (uint32_t)lorawan_pars.devaddr[1];
            devaddr = (devaddr << 8) | (uint32_t)lorawan_pars.devaddr[2];
            devaddr = (devaddr << 8) | (uint32_t)lorawan_pars.devaddr[3];
            u1_t nwk[16];
            u1_t app[16];
            ...
            seqnoUp = lorawan_pars.fcnt[0];
            seqnoUp = (seqnoUp << 8) | lorawan_pars.fcnt[1];
            ...
            LMIC_enableChannel(lorawan_pars.channel);   // O Default deveria ser habilitar do 8 ao 15
            LMIC_setLinkCheckMode(0);
