...
#define I2C_PORT I2C_MAIN_BUS


#define BMP280_I2C_TIMEOUT_US  10000
...
void bmp280_write_array(uint8_t deviceAddress, uint8_t startRegisterAddress, uint8_t *data, uint8_t dataLength)
{
    ...
    i2c_write_timeout_us(I2C_PORT, deviceAddress, buffer, dataLength + 1, false, BMP280_I2C_TIMEOUT_US);
}
...
void bmp280_read_array(uint8_t deviceAddress, uint8_t startRegisterAddress, uint8_t *data, uint8_t dataLength)
{
    ...
    i2c_write_timeout_us(I2C_PORT, deviceAddress, &startRegisterAddress, 1, true, BMP280_I2C_TIMEOUT_US);
    

    // Lê os dados a partir dele
    //i2c_read_blocking(I2C_PORT, deviceAddress, data, dataLength, false);
    i2c_read_timeout_us(I2C_PORT, deviceAddress, data, dataLength, false, BMP280_I2C_TIMEOUT_US);
}
