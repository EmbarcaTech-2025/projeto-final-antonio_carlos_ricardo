...
static bool    index_last_sent_valid;    // se é true index_last_sent tem o indice do último envidao
static int32_t index_last_sent;          // se é false o último enviado é anterior a index_data_min
...
uint64_t storage_adm_init(){
    ...
    if(first_sent){
        index_last_sent_valid = false;
    }else{
        index_last_sent_valid = true;
        index_last_sent       = index_sent_max;
    }
...
static void data_min_plus_one(){
    ...
    if(index_last_sent_valid){
        if(index_last_sent == index_data_min){
            index_last_sent_valid = false;
        }
    }
...
int32_t storage_adm_get_next_sent(){
    ...
    if(index_last_sent_valid){
        if(index_last_sent == index_data_max){
            //index_last_sending = -1;
            index_last_sending = -1;
        }else{
            //index_last_senting = index_last_send + 1;
            //if(index_last_sending >= STORAGE_ADDRESS_DATA_N_ITEMS){
            //    index_last_sending = 0;
            //}

            index_last_sending = index_last_sent + 1;
            if(index_last_sending >= STORAGE_ADDRESS_DATA_N_ITEMS){
                index_last_sending = 0;
            }
        }
...
int32_t storage_adm_can_update_lest_send(){
    if(index_last_sending >= 0){
        index_last_sent_valid = true;
        index_last_sent       = index_last_sending;
        return index_last_sending;
    }else{
        return -1;
    }
