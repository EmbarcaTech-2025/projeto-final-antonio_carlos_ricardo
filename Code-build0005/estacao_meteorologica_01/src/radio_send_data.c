...
static void tcp_data_err_callback(void *arg, err_t err){
    if(DEB_PR_CORE0_WIFI_SEND_DATA) printf("tcp_data_err_callback\n");
    // Isso pode ser chamado sem chance de resposta posterior, então apenas atualize o estado.
    radio_post_data.radio_post_state = RADIO_POST_ST_DATA_SEND_ERROR;
...
static err_t tcp_data_recv_callback(void *arg, struct tcp_pcb *tpcb, struct pbuf *p, err_t err) {
    if(DEB_PR_CORE0_WIFI_SEND_DATA) printf("tcp_data_recv_callback\n");
...
static err_t tcp_data_sent_callback(void *arg, struct tcp_pcb *tpcb, u16_t len) {
    if(DEB_PR_CORE0_WIFI_SEND_DATA) printf("tcp_data_sent_callback\n");
...
static err_t tcp_connected_send_data(void *arg, struct tcp_pcb *tpcb, err_t err) {
    if(DEB_PR_CORE0_WIFI_SEND_DATA) printf("tcp_connected_send_data\n");
    if (err != ERR_OK) {
        radio_post_data.radio_post_state = RADIO_POST_ST_DATA_SEND_ERROR;
    ...
    radio_post_data.radio_post_state = RADIO_POST_ST_DATA_SENDING;
    radio_post_data.sent_len         = 0;

    // Define Callbacks
    tcp_err( tpcb, tcp_data_err_callback);
    tcp_recv(tpcb, tcp_data_recv_callback);
    tcp_sent(tpcb, tcp_data_sent_callback); // você implementa esse para marcar que tudo foi enviado
...
bool radio_send_data(char * jsont_to_send){
    if(DEB_PR_CORE0_WIFI_SEND_DATA) printf("radio_send_data\n");
    ...
    radio_post_data.total_len = snprintf(radio_post_data.buffer, sizeof(radio_post_data.buffer),
        ...
        "\r\n"
        "%s",
        POST_PATH_SEND_DATA, config_data.server_url, config_data.server_port, json_len, jsont_to_send);
    ...
    //uint32_t tamanho = tcp_sndbuf(radio_post_data.pcb);
    //printf("Tamanho=%d\n", tamanho);
    //printf("Radio_post_data.pcb->snd_queuelen=%d\n", radio_post_data.pcb->snd_queuelen);



    radio_post_data.sent_len         = 0;
    radio_post_data.radio_post_state = RADIO_POST_ST_DATA_CONNECTING;
    radio_post_data.start_post       = time_us_64();
    err_t err = tcp_connect(radio_post_data.pcb, &radio_dest_ip, config_data.server_port, tcp_connected_send_data);
    if (err != ERR_OK) {
    ...
    //sleep_us(10000);
    bool ret = radio_post_data.radio_post_state == RADIO_POST_ST_DATA_SENT_OK;
    radio_post_data.radio_post_state = RADIO_POST_ST_IDLE;
    return ret;
}