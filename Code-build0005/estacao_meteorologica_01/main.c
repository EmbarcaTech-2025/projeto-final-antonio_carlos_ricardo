...
static bool send_data(){
    // ja rodou core0_comm_give_next_data() e retornou o dado valido
    int16_t size = estation_data_json(&core_comm_estation_data, estation_data_buf_tmp);

    if(radio_send_est_data(estation_data_buf_tmp)){
        if(DEB_PR_CORE0_SEND_DATA){
            printf("Core0-SEND-Data %d OK size=%d, data: ", ++send_data_count, size);
        ...
        if(DEB_PR_CORE0_SEND_DATA){
            printf("Core0-SEND-Data ERROR: size=%d, data: ", ++send_data_count, size);
            printf(estation_data_buf_tmp);
            printf("\n");
        }
        return false;
...
static bool get_server_time(){
    uint64_t new_time;
    if(radio_get_time_r(&new_time)){
        core0_comm_new_time(new_time);
        if(DEB_PR_CORE0_UPDATE_TIME){
            printf("Core0-UPDATE-Time Update to: ");
            util_print_timestamp(new_time);
        }
        return true;
    }else{
        if(DEB_PR_CORE0_UPDATE_TIME){
            printf("Core0-UPDATE-Time FALHA no acesso ao servidor\n");
        }
        return false;
    }
}
...
int main()
{
    ...
    if(config || (!config_data.rtc_with)) menu_rtc();
    ...
    uint64_t dt_next_retry     =   1000000 * 10; 
    ...
    uint count = 0;
    printf("Start Core0 Main Loop\n");

    WiFiState last_wifi_st = WIFI_ST_DISCONNECTED;
    led_rgb_set_st(LEDRGB_ST_WIFI_ST_DISCONNECTED);
    while (true) {
        bool wifi_st = radio_update();
        ...
        if(time_us_64() >= t_next_show){
            ...
            //int size  = estation_data_json_full_fake(&core_comm_estation_data, estation_data_buf_tmp, count);
            printf("Core0-SHOW-Last Aq Data (size=%d): %s\n", size, estation_data_buf_tmp);
        }
